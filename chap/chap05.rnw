...
\begin{figure}[!ht]
\centering
<<field-gcps, fig = TRUE, echo = FALSE>>=
...
\begin{description}
 ...
 outdoor e a árvore (\it{Cedrella fissilis}), junto à rodovia federal BR158.
 ...
 alinhado (\SI{20}{\cm}) com a cerca que separa a floresta nativa do pomar de \it{Citrus}~sp.~localizado 
...
(\autoref{tab:chap05-soil-attr-val}). A reason for this could be that validation was performed considering only 
...
\begin{description}
 \item[\texttt{SOIL\_100b}] Shallow soil (\it{Re4}) with low to high base saturation covering mountainous 
 ...
 \item[\texttt{SOIL\_100c}] Association (\it{Re-C-Co}) of shallow soil with high base saturation located in
steep terrain (Solo Litólico Eutrófico relevo forte ondulado; Neossolo Litólico Eutrófico; Eutric
 ...
 \item[\texttt{SOIL\_100d}] Association (\it{TBa-Rd}) of deep, well-structured, low base saturation soil (Terra
 Bruna Estruturada álica; Nitossolo; Nitisol), and shallow soil (Solo Litólico; Neossolo Litólico; Leptosol).
  
 \item[\texttt{SOIL\_100e}] Shallow soil (\it{Rd1} and \textit{Re4}) with low to high base
...
those 
 identified in the soil survey, that is \it{TBa-Rd}, described above, and \it{C1}, which is composed
...
Covariates derived from \soilNew{} are presented below. Mapping unit \it{RY}, composed mainly of soils 
developed 
from fluvial deposits (Neossolo Flúvico; Fluvisol) does not appear due to the small area that it occupies.

\begin{description}
  \item[\texttt{SOIL\_25a}] Moderately deep soil (\it{PBAC}) derived from sedimentary rocks, with abrupt 
textural
  change and low base saturation (Argissolo Bruno-Acinzentado; Alisol).
  
  \item[\texttt{SOIL\_25b}] Deep soil (\it{PV}) derived from igneous rocks, with moderate textural gradient,
...
base
  ...
  \item[\texttt{SOIL\_25d}] Shallow soil (\it{RL}) with low to high base saturation (Neossolo Litólico 
...
agriculture
  ...
  \item[\texttt{SOIL\_25i}] This covariate includes all three mapping units (\it{RL}, \it{RL-RR}, and \it{RR})
  composed mainly of shallow soils (Neossolo Litólico and Neossolo Regolítico; Leptosol and Regosol).
  
  \item[\texttt{SOIL\_25j}] This covariate includes all four mapping units (\it{PV}, \it{RL}, \it{RL-RR}, and 
...
\demNew{} using windows of sizes (from left to right) $3 \times 3$, $31 times 31$, $127 times 127$, and $255 
\times 255$.}
...
\begin{description}
 \item[\texttt{LU1980a}] Native forest (\it{FS}), which is likely to have soils with higher fertility.
  
 \item[\texttt{LU1980b}] Animal husbandry (\it{H}), the second most important land use in the study area, which 
is
...
\begin{description}
 \item[\texttt{LU2009a}] Native forest (\it{FS}), as described above.
 
 \item[\texttt{LU2009b}] Shrubland (\it{SS}), which is likely to have a soil fertility level above those found 
in
 ...
 \item[\texttt{LU2009d}] Annual crop agriculture (\it{AA}), which is likely to present the lowest soil 
fertility 
...
as (a) \texttt{NDVI_30} and (b) \texttt{NDVI_5b} included in the Santa Maria dataset.}
\label{fig:chap05-sat-image}
\end{figure}

\begin{table}[ht]
 \caption{Comparison between satellite images produced by Landsat 5 TM and RapidEye and 
