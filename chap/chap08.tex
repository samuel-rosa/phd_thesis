...
improvements on CLHS, which are evaluated using synthetic data derived from a real-world study case in Santa 
...
\end{equation}\label{eqn:chap08-lmm}
...
marginal sampling strata, the first and last marginal sampling strata 
...
0), (0, 1, 2, 2, 0), (0, 2, 2, 1, 0)\}$. Ergo, CLHS will be unable to find the globally optimum allocation 
...
stratum is directly proportional to the number of sampling units (grids cells of a raster image) in that 
...
\end{equation}\label{eqn:chap08-cramer}
...
\end{equation}\label{eqn:chap08-chi-squared}
...
\end{equation}\label{eqn:chap08-utility}
...
\end{equation}\label{eqn:chap08-corr}
...
\end{equation}\label{eqn:chap08-dist}
...
likelihood fit of the variogram model (red line) to BUDE, i.e. the stochastic component $e(\boldsymbol{s})$. 
...
\begin{equation} % SEE IMAGE BELOW.
 ...
 \end{cases}
\end{equation}\label{eqn:chap08-metropolis}
...
$\boldsymbol{f}(\boldsymbol{X}_{i + 1})$ and $\boldsymbol{f}(\boldsymbol{X}_i$ will be smaller, as well as the 
probability of accepting $\boldsymbol{f}(\boldsymbol{X}_{i + 1})$ if it is inferior to 
...
\item Larger sample sizes improve prediction quality irrespective of the sampling optimization algorithms, 
indicating that the sample size is more important than the sampling algorithm;
\item Sample configurations optimized aiming at the marginal distribution of the covariates have a more uniform 
coverage of the geographic space compared to aiming at the association/correlation between covariates, possibly 
