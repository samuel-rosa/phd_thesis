...
\chapternote{Collaborated in the preparation of this document: Pablo Miguel (UFPel), Jean Michel Moura Bueno 
...
containing the number of laboratory replicates is identified with the code of the soil property followed by the 
letter \q{N}. The column containing the sample standard deviation is identified in the same manner, but using 
\q{SD}. For example, \texttt{ORCA\_N} and \texttt{ORCA\_SD}.
...
\emph{topsoil}. The topsoil was defined as the topmost soil layer, with a depth equal or inferior to 
