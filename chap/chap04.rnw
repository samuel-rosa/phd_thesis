...
The first subset ($n = 340$, \autoref{fig:soil-data-subsets-I-III}) was produced as part of projects that 
aimed 
...
positional 
horizontal error of \SI{\pm6}{\metre}(\autoref{tab:chap05-covar-data-google-geo-val}).
...
husbandry (grasslands), forestry (\textit{Eucalyptus spp.} and \textit{Pinus spp.}), and row crop agriculture. 
Other 
...
(\texttt{ROCK} -- SIZE???). The quantity of coarse fragments (\texttt{GRAVEL}, \si{\percent}) was estimated 
visually in some observation points.
