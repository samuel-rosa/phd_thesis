...
how field description was done for subset III is given by \citeonline{Pedron2005} and \citeonline{Miguel2010}.
Merging field descriptions from subset III with those of subset I and II was preceded by conceptual 
translations if required -- this practice is reported when used.
...
uses were identified using nomenclature of \cite{FAO2006}:
...
Underlying geologic formation (\texttt{GEO}) and lithology (\texttt{LITHO}) were deduced based on soil 
properties and environmental features observed in the field, and on existing area-class soil maps 
(\autoref{sec:sec:chap05-soil-maps}) geologic maps (\autoref{sec:sec:chap05-geo-maps}).
...
soil properties (\SI{20}{\centi\metre}-deep soil pits and auger holes down to the diagnostic subsurface 
horizon or bedrock) and local environmental features. Soil taxon was inferred only to the second categorical 
level of the Brazilian System of Soil Classification \cite{SantosEtAl2013a} because further levels require 
data that were not observed in the field.
