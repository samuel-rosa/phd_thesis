...
A lot happened since the Neolithic Revolution \citep{BrevikEtAl2010} -- from bone to satellite, as 
...
of agriculture and increase of food production. An important step was given about 150 years ago with 
the formalization of the approximate understanding of the spatial soil variation using 
scientific parlance (i.e. mathematization) \citep{Florinsky2012}, which was revisited 
ca.~\num{75}~year ago \citep{Jenny1941}. Soil maps produced during this long period of human history
fit into what we call today the \texit{discrete model of spatial variation} \citep{Heuvelink1996}. The 
discrete model of spatial variation explains the variation of soil properties in space using 
mutually exclusive mapping units that are separated by sharply defined, crisp boundaries (i.e. 
polygons) \citep{Legros2006}. The soil in each mapping unit is more or less homogeneous with regard 
to its properties at the time in which the map was produced. These properties, which are generally 
used to name the mapping unit, can be characterized using one or more direct observations made 
within the domain of the mapping unit \citep{WebsterEtAl1990, Legros2006}.

Postwar scientific/technological developments provided the tools to deal with the main weakness of
the discrete model of soil variation: the description of ``soil bodies'' as discrete, homogeneous 
entities \citep{ScullEtAl2003}. Such weakness was evidenced by the fact that different soil 
scientists would produce considerably different soil maps \citep{Legros2006}. As such, many soil 
scientists decided to employ the new developments in the fields of mathematics, statistics, and 
informatics, already successfully explored by the mining industry, to explain the spatial variation 
of soil properties \citep{Matheron1969}. The new model, the \textit{continuous model of spatial 
variation} assumes that the spatial variation of soil properties is gradual and depends only on the 
spatial distances \cite{WebsterEtAl1990, Heuvelink1996}.
