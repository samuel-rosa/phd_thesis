...
Soil Governance Conference}, where the National Program for Soil Survey and Interpretation of Brazil 
...
body of knowledge on soil management and conservation \citep{Blanco-CanquiEtAl2010}. One could also 
argue whether soil information is really needed to find a solution to major issues such as 
...
-- it is long known that about \SI{50}{\percent} of what is produced is lost, even in rich countries
such as Switzerland \citep{BerettaEtAl2013}.



In Brazil, the main reason for restarting the national soil survey program, today called PRONASOLOS,
seems to be the same reason of about half a century ago: the (inter)national pressure to occupy the 
the Cerrado and Amazon biomes

, specially in the north and north-eastern states of Marranhão, Tocantins,
Piauí, and Bahia (MATOPIBA)  into agricultural land 
