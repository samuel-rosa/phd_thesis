...
covariates $\boldsymbol{x}$. The random effects, $e(\boldsymbol{s})$, describe the portion of the 
spatial variation of the soil property that cannot be explained with the covariates but is 
spatially correlated, the form and degree of 
this spatial correlation possibly being interpreted pedologically \citep{Lark2012}. 


assume to be a linear function of the predictor variables. The random 
...
$\beta_{0}$ is the intercept.}
 ...
 possessing a large flexibility that allows exploring newly developed 
(geo)statistical and data-mining methods, generally resulting in better 
performance than the constituent models alone \citep{OdehEtAl1994, OdehEtAl1995, Heuvelink1996, 
McBratneyEtAl2000, HenglEtAl2004, Lopez-GranadosEtAl2005, Lark2012}. These features, and the 
development of the Internet, promoted the rapid popularization of the mixed model of spatial 
variation. Many recent regional to large scale soil-mapping projects already employed the mixed 
model of spatial variation \citep{PoggioEtAl2014, NussbaumEtAl2014, HenglEtAl2015}, .

Some seem to suggest that the mixed model of spatial variation caused a paradigm shift in soil 
science \citep{McBratneyEtAl2003} and that it is the ultimate soil mapping method 
\citep{MinasnyEtAl2016}. This is certainly untrue in many poorer regions where the mixed model of 
spatial variation seems useless because farmers already obtain high yields and properly manage their
soils using indigenous/local knowledge to map the soil \citep{Barrera-BassolsEtAl2003, 
Barrera-BassolsEtAl2006, HillyerEtAl2006, CorreiaEtAl2007, ValeJuniorEtAl2007}. Others have strongly
criticized soil scientist that refused to adopt the mixed model of spatial variation, sometimes 
using (pejorative) arguments \citep{HeuvelinkEtAl2001, Mendonca-SantosEtAl2003} that helped building
a negative atmosphere among soil scientists in many countries (see examples from Brazil 
[\url{https://groups.google.com/forum/#!forum/soil-mapping}] and Spain 
[\url{http://www.madrimasd.org/blogs/universo/2009/10/07/126094}]). Perhaps the staunch supporters 
of the mixed model of spatial variation were unable to see that most of the other soil scientists 
simply had a poor knowledge of mathematics, statistics, informatics, and English 
\citep{SamuelRosa2012}. Although this \textit{discriminatory behaviour} is common when scientists
want to make a new theory or method accepted \citep{Russell1932, Feyerabend1977, Kuhn2011}, a point
has been reached in which nothing else can be gained by playing one soil scientists against the other.
Such a noble understanding was already shared by \citet{Jenny1941} when comparing ``soil 
geographers'' and ``soil functionalists''.
