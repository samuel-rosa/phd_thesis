...
\subsection{Need for soil information}
...
fundamentally was an economic decision in which the general public was not given the chance to 
participate \citep{SamuelRosa2012}.
...
\citep{Kempen2011}. I suggest that soil is now a hot topic. For example, the United Nations 
(\href{http://www.un.org/apps/news/story.asp?NewsID=49520#.VnremV6alz0}{UN}) declared 5 December 
the World Soil Day and 2015 the International Year of Soils ``\textit{in an effort to raise 
awareness and promote more sustainable use of this critical resource}''. A global consortium, the 
\href{http://www.globalsoilmap.net/}{GlobalSoilMap}, was created with the goal of producing 
``\textit{a new digital soil map of the world using state-of-the-art and emerging technologies}''. 
...
``\textit{leading to the adoption of sustainable development goals for soils}''. The International 
Union of Soil Sciences created a working group, funded by the United States Department of Agriculture 
(\href{http://www.nrcs.usda.gov/wps/portal/nrcs/main/soils/survey/class/}{USDA}) to develop a 
Universal Soil Classification System, ``\textit{a common language to describe soils that can be used 
...
was composed, counting with soil experts from all regions of the world ``\textit{to provide 
...
\& Melinda Gates} foundation handed out an \SI{18}[\$]~million grant ``\textit{to map most parts in 
...
``\textit{framework for production of open soil data}''. In Brazil, soil scientists created the 
Brazilian Network for Research in Digital Soil Mapping (\href{https://goo.gl/m8QWUm}{RedeMDS}) with 
the objective of ``\textit{generating synergy among Brazilian soil scientists to advance the 
...
to reach about nine billion people by \num{2050} \citep{SanchezEtAl2009}. But FAO has already recognized 
...
Brazilian states that compose ``the last agriculture frontier'', called MATOPIBA (States of 
...
SECOM2015}. It is in this contradictory scenario that I present this thesis!

In the next two sections I give an overview of the models of spatial variation used for soil mapping
(\refsec{intro-soil-mapping}) and discuss about some of the main sources of uncertainty in
...
for taxation purposes \citep{Barrera-BassolsEtAl2003} -- a practice that lasts till today.
...
conditions \citep{MazoyerEtAl2008}. If we adopt an integrative view, soil maps produced during this 
long period of human history seem to fit into what we call today the \texit{discrete model of 
...
crisp boundaries (i.e. polygons) \citep{Heuvelink1996, Legros2006}. The soil in each mapping unit 
is more or less homogeneous with regard to its properties at the time of mapping. These properties, 
which are generally used to name the mapping unit, can be characterized using one or more direct
observations made within the domain of the mapping unit \citep{WebsterEtAl1990, Legros2006}.
...
\noindent
i.e. a soil property $s$ is \textit{determined} by the interplay of environmental conditions defined
by climate ($cl$), organisms ($o$), relief ($r$), parent material ($p$), time ($t$), and other unknown 
players ($\ldots$) \citep{Jenny1941}. When \citet{Jenny1941} formulated \autoref{eqn:intro-clorpt},
...
\citet{Jenny1941} proposed that solving \autoref{eqn:intro-clorpt} depended on the soil scientist' 
...
required the soil-forming factors to be exhaustively known everywhere. \autoref{eqn:intro-clorpt} 
...
\citep{Schelling1970, Hudson1992, BockheimEtAl2000, Legros2006, KrasilnikovEtAl2009b, 
HarteminkEtAl2013}.
...
\citep{HeuvelinkEtAl2001, McBratneyEtAl2003, ScullEtAl2003}:
...
\citep{WebsterEtAl1990} -- this is the so-called field of geostatistics.

Soil maps were now produced using a computer, requiring soil-mapping rules to be formalized in the 
form of a computer script, which is the mean used to establish the communication between the soil 
modeller/mapper (a human being) and the data processing environment (a computer) where 
geostatistical models are implemented. This meant that soil-mapping rules could be effectively 
shared with others -- provided they had a computer. Taking the uncertainty about the mapped soil 
property into account was also possible. The only requirement was that a soil property be treated as
a stochastic variable \citep{Cressie1993}, i.e. as if it was impossible to be completely certain 
about its absolute value but only to have an idea of its most likely value. As such, the definition 
of mapping units, which was based on using the knowledge of the soil-forming factors, could be 
viewed as a process that aims at minimizing the within-unit variance (and maximizing the between-unit
variance) of that stochastic variable \citep{VoltzEtAl1990}. Accordingly, the most likely value of a
soil property in a given mapping unit is the mean over the multiple observations made in that 
mapping unit \citep{VoltzEtAl1990, Cressie1993}. It follows that the uncertainty about the value of
the mapped soil property at a given location is a measure of how much in error we are if the true
value is replaced with the mean of all values observed in the mapping unit, i.e. is the same 
everywhere.

The only solution for avoiding the description of ``soil bodies'' as discrete, homogeneous entities 
was to ignore the soil-forming factors and use the \textit{continuous model of spatial variation}.
...
coordinates \citep{WebsterEtAl1990, Cressie1993}. In the simplest case, the most likely value of a 
...
the separation distance between locations \citep{Cressie1993}. The main idea underlying the 
continuous model of spatial variation is that soil properties at nearby locations are more similar 
than at locations that are far apart \citep{WebsterEtAl1990}. Thus, we err less if we replace the
...
spatially varying \citep{Cressie1993}.
...
\citep{HeuvelinkEtAl2001, McBratneyEtAl2003, ScullEtAl2003}. But limitations in its prediction 
...
example, between agricultural fields, parent materials, land uses, and so on \citep{SteinEtAl1988,
VoltzEtAl1990}. Because, contrary to the discrete model of spatial variation, the continuous model 
of spatial variation largely ignores the existing pedological knowledge \citep{Grunwald2009, 
Lark2012}. Soil scientists also understood that the discrete model of spatial variation was more 
efficient than previously thought \citep{BregtEtAl1987}. First, because it was now possible to 
employ Jenny's equation of soil-forming factors for soil mapping using remote sensing products as 
surrogates of the factors of soil formation \citep{MooreEtAl1993}. Second, machine-learning 
algorithms enabled identifying complex spatial patterns that before could only be identified by an 
experienced soil scientist \citep{McKenzieEtAl1999}. The most logical step was to combine the 
strengths of both discrete and continuous models of spatial variation into a single model -- the 
\textit{mixed model of spatial variation} --, that is inclusion of the existing pedological 
...
predictive soil mapping, and so on \citep{Hengl2003, McBratneyEtAl2003, ScullEtAl2003} -- can be 
...
spatial stochastic process \citep{Cressie1993, HeuvelinkEtAl2001, LarkEtAl2006}. Accordingly, the 
model is composed of \textit{fixed} and \textit{random effects}. The fixed effects,
a deterministic large-scale spatial trend, $m(\boldsymbol{s})$, describes the portion of the spatial
...
covariates. The random effects, also known as stochastic residuals or latent 
variables, $e(\boldsymbol{s})$, describe the portion of the spatial variation of the soil property 
that cannot be explained with the covariates but is spatially correlated, the form and degree of 
this spatial correlation possibly being interpreted pedologically \citep{Lark2012}. Thus

\begin{equation}\label{eq:intro-mixed-model}
 Y(\boldsymbol{s}) = m(\boldsymbol{s}) + e(\boldsymbol{s}).
\end{equation}

\autoref{eq:intro-mixed-model} possesses a great flexibility that makes easy to explore newly 
developed (geo)statistical and data-mining methods, generally resulting in better performance than 
the constituent models alone, as well as integrating the existing pedological knowledge 
\citep{OdehEtAl1994, OdehEtAl1995, Heuvelink1996, McBratneyEtAl2000, HenglEtAl2004, 
Lopez-GranadosEtAl2005, WebsterEtAl2007, Grunwald2009, Lark2012}. These features, and the 
development of the Internet, promoted the rapid popularization of the mixed model of spatial 
variation, and many recent regional to large scale soil-mapping projects already employed the mixed 
model of spatial variation with success \citep{PoggioEtAl2014, NussbaumEtAl2014, HenglEtAl2015}.

Unfortunately, along with the success of the mixed model of spatial variation, came a rupture 
between soil scientists pertaining to different schools, building a negative atmosphere in many 
...
represented a ``paradigm shift'' in soil science \citep{McBratneyEtAl2003} and that it is the 
ultimate soil mapping method, superior to all others \citep{MinasnyEtAl2016}. This assumption is 
certainly untrue, specially in many poorer regions where the mixed model of spatial variation seems 
useless because farmers already obtain high yields and properly manage their soils using 
indigenous/local knowledge \citep{Barrera-BassolsEtAl2003, Barrera-BassolsEtAl2006, HillyerEtAl2006, 
CorreiaEtAl2007, ValeJuniorEtAl2007}. Perhaps the strong criticism made against soil scientist that 
refused to adopt the mixed model of spatial variation, using somewhat pejorative arguments 
\citep{HeuvelinkEtAl2001, Mendonca-SantosEtAl2003}, helped building this unfruitful scenario. On 
the other side, soil scientists disliked being pushed away from their research field, perhaps being 
afraid of the new technological developments due to their poor knowledge of mathematics, statistics,
and informatics \citep{Webster2001, SamuelRosa2012}.

Although all that is very common when a new theory or method appears \citep{Russell1932, 
Feyerabend1977, Kuhn2011}, I think we have reached a point in which nothing else can be gained by 
playing one soil scientist against the other. Such a (noble) understanding was already shared by 
\citet{Jenny1941} when comparing ``soil geographers'' and ``soil functionalists''. The view that I
try to maintain from beginning to end of this thesis is that, despite the technological developments,
the activity of mapping the soil remained essentially the same throughout human history. Soil maps 
still serve the same old purpose of representing our limited understanding about the spatial 
organization of the soil in the natural environment in a simplified manner, as well as giving 
insights about how the soil came to be \citep{Hudson1992, Legros2006, Grunwald2010}. In support to 
this view, based on the existing body of knowledge on soil mapping and the currently available 
technology, I have tentatively defined a general (didactic) sequence of seven steps that I believe 
are (or should be) commonly followed in a soil-mapping project.

\noindent\textit{Step 0}. Identify a reality or problem entity, the geographic region for which 
there is a demand of spatial soil information. Target soil properties (categorical, ordinal, 
continuous, binary, etc.) are appointed as well as the required accompanying output information 
(e.g. metadata). Key modelling decisions are taken in this step such as the support (punctual or 
areal), spatial resolution (and possibly the cartographic scale), coordinate reference system, etc. 
Depending on how well defined the demand is, the conceptual model of spatial variation can also be 
specified, i.e. discrete, continuous, or mixed. Data policy is discussed (What data should the 
public? How to make data public? How to implement the data policy?) and agreed upon. Finally, the 
available infrastructure, budget, time, and workforce are specified so that next steps can be 
...
books, websites, as well as existing maps of the soil, land use, geology, digital elevation models, 
satellite images, aerial photographs, among others. Environmental information is used to articulate 
the concepts of a chosen pedological model of explanation of soil spatio-temporal variation. 
Provided that any of the existing soil data is available, an exploratory data analysis can help 
unravelling soil-landscape relationships. The poorer the volume of existing environmental 
information, and the less experienced the soil modeller/mapper is, the more import an exploratory 
field campaign is to help understanding the existing soil-landscape relationships.

\noindent\textit{Step 2}. Define the conceptual model of spatial variation, a translation of the 
conceptual model of pedogenesis into a set of possible mathematical representations. Depending on 
how well defined the demand was, the conceptual model of spatial variation was already specified in 
...
allows carrying out exploratory field campaigns, a single conceptual model of spatial variation is 
defined, i.e. discrete, continuous, or mixed. Assuming that the mixed model of spatial variation is 
chosen, the (geo)statistical and/or data-mining models that will be used to represent the discrete 
...
conceptual models or (geo)statistical and/or data-mining models are chosen, the pedological and 
...
needed to estimate the parameters and validate the chosen (geo)statistical and/or data-mining
...
for non-linear transformations to meet the requirements of the chosen (geo)statistical and/or 
...
\noindent\textit{Step 4}. Estimate the parameters of the (geo)statistical and/or data-mining 
...
computer code or script. The computer code is used to establish the communication between the 
soil modeller/mapper (a human being) and the data processing environment (a computer) where 
the mathematical representations and model assessment tools are implemented. Developing a well
documented computer code that describes all processing steps ease re-design, future consistency 
checks, correction of mistakes, and dissemination/reproducibility. Calibrated models are evaluated 
using statistical criteria defined in \textit{Step 2} such as goodness-of-fit measures. Best 
performing models are evaluated regarding their tenability (pedological evaluation), which includes
visually assessing draft soil maps, and how well they represent the range of possible mathematical
models. Failure in this last assessment suggests that the model requires adjustments, possibly 
more calibration data, or that it can be discarded.

\noindent\textit{Step 5}. Validate the (geo)statistical and/or data-mining models, preferentially
