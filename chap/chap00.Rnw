...
conditions. Soil maps produced during this long period of human history seem to fit into what we 
call today the \texit{discrete model of spatial variation}. The discrete model of spatial variation 
explains the variation of soil properties in space using mutually exclusive mapping units that are 
separated by sharply defined, crisp boundaries (i.e. polygons) \citep{Heuvelink1996, Legros2006}. 
The soil in each mapping unit is more or less homogeneous with regard to its properties at the time 
in which the map was produced. These properties, which are generally used to name the mapping unit, 
can be characterized using one or more direct observations made within the domain of the mapping 
unit \citep{WebsterEtAl1990, Legros2006}.
...
i.e. a soil property is \textit{determined} by the interplay of environmental conditions defined by 
climate ($cl$), organisms ($o$), relief ($r$), parent material ($p$), time ($t$), and other unknown 
players ($\ldots$) \citep{Jenny1941}. The goal of \citet{Jenny1941} was to organize the large volume
of already existing soil data/knowledge by means of numerical laws and quantitative theories -- 
instead of soil maps, taxonomic classifications, and soil-forming processes -- to enable treating 
it mathematically (i.e. empirical correlation). \citet{Jenny1941} proposed that solving 
\autoref{eqn:intro-clorpt} depended on the soil scientist' skills to select suitable study areas 
and locations for making observation. He also new that direct application of 
\autoref{eqn:intro-clorpt} for soil mapping was impossible at his time because it required the 
soil-forming factors to be exhaustively known everywhere. The concept of soil-forming factors was 
...
determined by soil-forming factors had its weaknesses. Those that today are pointed as the three 
main weaknesses were recognized or understood only using postwar scientific/technological 
developments \citep{Webster1994, HeuvelinkEtAl2001, McBratneyEtAl2003, ScullEtAl2003}:
...
Producing soil maps using a computer required soil-mapping rules to be formalized in the form of a 
computer script. This meant that soil-mapping rules could be effectively shared with others -- 
provided they had a computer and understood geostatistics. Taking the uncertainty of the mapped
soil property was also possible. The only requirement was that a soil property be treated as a 
stochastic variable \citep{Cressie1993}, i.e. as if it was impossible to be completely certain about
its absolute value but only to have an idea of its most likely value. As such, the definition of 
mapping units, which was based on using the knowledge of the soil-forming factors, could be viewed 
as a process that aims at minimizing the within-unit variance, and maximizing the between-unit 
variance \citep{VoltzEtAl1990}. Accordingly, the most likely value of a soil property in a mapping 
unit, and its associated uncertainty, is defined as it mean and variance over the multiple 
observations made in the mapping unit \citep{VoltzEtAl1990, Cressie1993}. 

The solution for eliminating the description of ``soil bodies'' as discrete, homogeneous entities
is to use the \textit{continuous model of spatial variation}, which states that the spatial 
variation of soil properties is gradual and depends only on the spatial distances, and possibly on a
spatial trend defined by the geographic coordinates \citep{WebsterEtAl1990, Cressie1993}. In the 
simplest case, most likely value of a soil property in a given location is defined as the mean over 
all observations made in the mapping region, plus a random variable wit mean zero and spatial 
(autoco)variance that depends on the separation distance between two locations \citep{Cressie1993}.
The main idea underlying the continuous model of spatial variation is that soil properties at nearby
locations (short separation distances) are more similar (small autocovariance) than at distant 
locations (large autocovariance). It follows that the uncertainty about the mapped soil property is 
larger the farther from existing soil observations \citep{Cressie1993}.

Availability of general-purpose computers fuelled the use and development of the continuous model of
spatial variation in rich European, North American, and Oceanic countries \citep{HeuvelinkEtAl2001, 
McBratneyEtAl2003, ScullEtAl2003}. But recent developments in remote sensing and machine-learning 
algorithms helped many soil scientists to understand that the continuous model of spatial variation 
had limitations too. For instance, it is unable to capture abrupt changes in the values of soil 
properties that occur, for example, at the boundaries of agricultural fields \citep{VoltzEtAl1990}. 
Besides, contrary to the discrete model of spatial variation, it largely ignores the existing 
pedological knowledge \citep{Grunwald2009, Lark2012}. Soil scientists also understood that the 
discrete model of spatial variation was more efficient than previously thought. First, because it 
was now possible to employ Jenny's equation of soil-forming factors for soil mapping using remote 
sensing products as surrogates of the factors of soil formation \citep{MooreEtAl1993}. Second, 
machine-learning algorithms enabled identifying complex spatial patterns that before could only be 
identified by an experienced soil scientist \citep{Grunwald2009}. The new model -- the \textit{mixed
model of spatial variation} -- combined the strengths of both discrete and continuous models of
spatial variation, that is inclusion of the existing pedological knowledge and consideration of the 
spatial continuity soil property values, as well as included newly developed technologies, generally
resulting in a better performance than its constituent models \citep{OdehEtAl1994, OdehEtAl1995, 
Heuvelink1996, McBratneyEtAl2000, HenglEtAl2004, HenglEtAl2015}.
