...
\SweaveOpts{prefix.string = fig/chap00} % tell Sweave where to put figures
...
\begin{figure}[!ht]
\centering
<<intro-database, fig = TRUE, echo = FALSE, width = 90mm>>=
cal_data <- read.table(
...
pts <- data.frame(cal_data@coords)
...
spgrass6::initGRASS(
  ...
  override = TRUE)
pol <- spgrass6::readVECT6("buffer_BASIN_10")
drain <- spgrass6::readVECT6("STREAM_10")
...
obtained from the Santa Maria dataset used in this thesis. The drainage network is shown in the 
...
lattice::trellis.par.set(
  ...
  layout.widths = list(left.padding = 0, right.padding = 0), 
  layout.heights = list(top.padding = 0, bottom.padding = 0))
aa <- plotHD(tmp$CLAY, xlab = "CLAY")
bb <- plotHD(tmp$ORCA, xlab = "ORCA")
cc <- plotHD(tmp$ECEC, xlab = "ECEC")
dd <- plotHD(na.exclude(tmp$BUDE), xlab = "BUDE")
@
