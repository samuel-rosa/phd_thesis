...
\subsection{Soil mapping models}
...
conditions \citep{MazoyerEtAl2008}. Soil maps produced during this long period of human history seem
to fit into what we call today the \texit{discrete model of spatial variation}. The discrete model 
of spatial variation explains the variation of soil properties in space using mutually exclusive 
mapping units that are separated by sharply defined, crisp boundaries (i.e. polygons) 
\citep{Heuvelink1996, Legros2006}. The soil in each mapping unit is more or less homogeneous with 
regard to its properties at the time of mapping. These properties, which are generally used to name 
the mapping unit, can be characterized using one or more direct observations made within the domain 
of the mapping unit \citep{WebsterEtAl1990, Legros2006}.
...
skills to select suitable study areas and locations for making observation. He also new that direct 
...
\citep{WebsterEtAl1990} -- the so-called field of geostatistics.
...
modeller/mapper (a human being) and the data processing environment (a computer) where the 
...
mapping unit \citep{VoltzEtAl1990, Cressie1993}. It follows that the uncertainty about the mapped 
soil property in a mapping unit is a measure of how much in error we are if an observed value is 
replaced with the mean of all values observed in the mapping unit, i.e. is the same everywhere.
...
than at locations that are far apart \citep{WebsterEtAl1990}. Thus, we err less if we replace an 
observed value with a value observed at a nearby location than with a value observed at a distant 
location. It follows that the uncertainty about the mapped soil property is larger the farther from 
existing soil observations, i.e. it is spatially varying \citep{Cressie1993}.

Availability of general-purpose computers fuelled the use and development of the continuous model of
spatial variation in rich European, North American, and Oceanian countries \citep{HeuvelinkEtAl2001, 
McBratneyEtAl2003, ScullEtAl2003}. But limitations in its prediction performance and recent developments
in remote sensing and machine-learning algorithms helped many soil scientists to understand that the
continuous model of spatial variation had limitations too. For instance, it is unable to capture 
abrupt changes in the values of soil properties that occur, for example, between agricultural fields,
parent materials, land uses, and so on \citep{SteinEtAl1988, VoltzEtAl1990}. Because, contrary to 
the discrete model of spatial variation, the continuous model of spatial variation largely ignores 
the existing pedological knowledge \citep{Grunwald2009, Lark2012}. Soil scientists also understood 
that the discrete model of spatial variation was more efficient than previously thought 
\citep{BregtEtAl1987}. First, because it was now possible to employ Jenny's equation of soil-forming
factors for soil mapping using remote sensing products as surrogates of the factors of soil formation
\citep{MooreEtAl1993}. Second, machine-learning algorithms enabled identifying complex spatial 
patterns that before could only be identified by an experienced soil scientist \citep{McKenzieEtAl1999}.
The most logical step was to combine the strengths of both discrete and continuous models of spatial
variation into a single model -- the \textit{mixed model of spatial variation} --, that is inclusion
of the existing pedological knowledge and consideration of the spatial continuity of soil property 
values.
...
spatial stochastic process \citep{Cressie1993, HeuvelinkEtAl2001, LarkEtAl2006, NussbaumEtAl2014}. 
Accordingly, the model is composed of \textit{fixed} and \textit{random effects}. The fixed effects,
...
covariates $\boldsymbol{x}$. The random effects, also known as stochastic residuals or latent 
...
insights about how the soil came to be \citep{Hudson1992, Legros2006, Grunwald2010}. As such, based
on the currently existing body of knowledge on soil mapping, I have tentatively defined a general 
sequence of seven steps taken in a soil mapping project.
