...
% Accurately capturing the structure of the large scale soil spatial variation depends on the form of the 
% model, but also on the available soil and covariate data (see \autoref{sec:chap02-covar-data} and 
% \autoref{sec:chap02-soil-data}). The same is true when modelling the structure of the small scale soil 
% spatial variation using the continuous model of spatial variation (or the stochastic component of the mixed 
% model of spatial variation). Here the soil modeller has to decide upon the form of the spatial covariance 
% function. The number of decisions required increases if the choice is for using the mixed model of spatial 
% variation to model both deterministic and stochastic components of soil spatial variation. 

% The decision can be based on the existing pedological knowledge and on using sound statistical inference 
% made on external validation data. 
