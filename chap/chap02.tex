...
\gates{} foundation handed out an 18~\$~million grant \cited{to map most parts in Sub-Saharan Africa, and 
...
was announced, with an expected budget of 8~R\$~billion\footpronassolos. In the sequence, Embrapa Soils 
...
location of existing soil observations \cite{Heuvelink1996}. This is because the expected error of predicting 
the value of a soil property at any location with its mean is also the same everywhere.
...
The obvious difficulty in this approach is that the PDF cannot be defined because we usually have one single 
value recorded at each observation location.
...
defined a general (didactic) sequence of seven steps that I believe should be
...
needed to estimate the parameters and testvalidate the chosen statistical and/or data-mining
...
aggregated version of these models, or parsimony is considered to elect the simpler model with fewer variables, 
steps, rules, etc. If previous steps have already allowed selecting a single best
...
knowledge, it will never be possible to construct a model that explains the entire complexity of
...
map, they can always be considered as wrong, and there can be many alternative versions for the same map, the 
...
must be careful not to decide upon collecting an insufficient or an exaggerated number of soil samples.
Under-sampling can result in soil models with little utility, while over-sampling can produce
...
every soil modeller with long-term practical experience in soil mapping \cite{MeyerEtAl2001}.
...
consistently suffers from natural and/or anthropogenic alterations is geomorphologically more complex than
...
to (forward selection) or removed from (backward elimination) the model \cite{LarkEtAl2007a}. The stepwise 
method adds and removes covariates until no further addition or removal results in
...
of covariates which may have a weaker physical or biological relation with the soil property being 
modelled/mapped.
