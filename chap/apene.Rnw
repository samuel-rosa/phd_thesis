...
the approximate taxonomic composition (\si{\percent}). Validation statistics are absent in the survey report.
...
\begin{table}[ht]
 ...
 \begin{tabular}{lrrrr}
  ...
  \hline
  Mean, \si{\m}               & 30   (79)    & -36  (67)    & 105   (43)   & \ang{128} (\ang{80}) \\ 
  Absolute mean, \si{\m}      & 58   (63)    & 64   (40)    & -            & -                    \\ 
  Squared mean, \si{\m\square}& 7241 (11353) & 5712 (6197)  & 12953 (9613) & -                    \\ 
  \hline
 \end{tabular}}
\end{table}
...
considerably in their positional accuracy (\autoref{sec:covar-data-dem} and 
\autoref{sec:covar-data-land-use}). To avoid data losses, all boundary gaps were manually filled using the 
closest mapping unit. Boundaries of soil polygons were defined based on land use (\landNew{}, 
\autoref{sec:covar-data-land-use}) and topographic data 
...
Both \soilOld{} and \soilNew{} were imported into GRASS GIS, cropped to the bounding box of the study area, 
and geometrically corrected to match the prediction grid (\SI{5}{\metre} grid size). Registration and geocoding 
was performed using the nearest neighbour resampling method to maintain data integrity. Each category was named 
with the code of the respective mapping unit in the original soil map. Prior to validation in the attribute 
...
\soilNew{} would outperform \soilOld{} if validation data included soil 
...
potential sources of error. First, because \soilOld{} does not include analytical soil data 
...
\begin{table}[ht]
 \caption{Estimated error statistics of the validation of area-class soil maps \soilOld{}
 and \soilNew{} in the attribute space. Validation statistics were estimated using $n = 60$ 
 ...
 \begin{tabular}{lrrr}
  ...
  \hline
  \soilOld{}    & 21.69    & 31.67    & 41.65    \\
  \soilNew{}     & 20.81    & 30.00    & 39.19    \\
  \hline
 \end{tabular}}
\end{table}
...
The main advantage of \soilNew{} in relation to \soilOld{} is the level of detail. 
While \soilOld{} has only five mapping units covering catchment of the DNOS/CORSAN 
reservoir, \soilNew{} has seven mapping units. This enabled the derivation of six 
covariates from \soilOld{} and ten covariates from \soilNew{}. Covariates derived 
from \soilOld{} are the following:
