...
\begin{figure}[!ht]
 ...
 <<covar-data-land-use-1, fig = TRUE, echo = FALSE>>=
  ...
  # Prepare plot
  map <- spplot(
    map, main = "", col.regions = worldgrids_pal$glc2000[c(1, 13, 4, 22)], asp = 1) +
      latticeExtra::layer(sp::sp.points(pts, col = "red", pch = 20))
  names(map$legend) <- "inside"
 ...
 <<covar-data-land-use-2, fig = TRUE, echo = FALSE>>=
  ...
  # Prepare plot
  map <- spplot(
    map, main = "", col.regions = worldgrids_pal$glc2000[c(1, 12, 13, 16, 4, 22, 20)], asp = 1) +
      latticeExtra::layer(sp::sp.points(pts, col = "red", pch = 20))
  names(map$legend) <- "inside"
  map$legend$inside$x <- 0.84
