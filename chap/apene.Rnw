% ATTENTION: 
% lines starting with “%”  will not be included in the document
% skip text (code) in red
...
existence of easily identifiable geographical markers across the covariates, including road intersection, 
fence 
...
\autoref{sec:covar-data-land-use}). 
To avoid data losses, all boundary gaps were manually filled using the closest mapping unit. Boundaries of 
soil 
...
delineation 
of mapping units was done manually, on the computer screen, without using any automated classification 
routine. 
