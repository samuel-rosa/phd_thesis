...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{95mm}
  \subcaption{}
  \label{fig:chap05-brazil}
  \centering
  \includegraphics[width=90mm]{fig/chap01FIG1a}
 \end{minipage}
 \begin{minipage}[b]{95mm}
  \subcaption{}
  \label{fig:chap05-points}
  \centering
  \includegraphics[width=90mm]{fig/chap01FIG1b}
 \end{minipage}
 \caption{Location of the study area in Santa Maria (a) and spatial distribution of the point soil observations
 and drainage network (b).}
 \label{fig:chap05-location}
\end{figure}
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2a}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2d}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2b}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2e}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2c}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG2f}
 \end{minipage}
 ...
 \label{fig:chap05-soil-properties}
\end{figure}

Point soil data, here denoted by $Y(s)$, showed a positive skew (\autoref{fig:chap05-soil-properties}) and was 
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{63mm}
  ...
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG3a}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{Cartographic scale: \num{1}:\num{25000}}
  \label{fig:chap05-soil-new}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG3d}
 \end{minipage}    
 \begin{minipage}[b]{63mm}
  \subcaption{Cartographic scale: \num{1}:\num{50000}}
  \label{fig:chap05-geo-old}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG3b}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{Cartographic scale: \num{1}:\num{25000}}
...
with \scale{50000} \cite{GasparettoEtAl1988} (\autoref{fig:chap05-geo-old}). The more detailed geologic map 
(\geoNew) was produced using topographic maps with \scale{25000}, and includes the location of overlaying 
Quaternary sedimentary deposits \cite{MacielFilho1990} (\autoref{fig:chap05-geo-new}). \geoNew{} did not cover 
...
predictor variables were derived from \geoOld{} and four from \geoNew{} (\autoref{tab:chap05-geology-covars}).
...
(\autoref{fig:chap05-land-old}). The more detailed land use map (\landNew) was prepared (\scale{2000}) by 
manual digitization using \SI{65}{\cm} spatial resolution satellite images covering the years \num{2008} and 
\num{2009} \cite{SamuelRosaEtAl2011a} (\autoref{fig:chap05-land-new}). Mapping units depicting human 
...
and two from \landOld{} (\autoref{tab:chap05-land-covars}).

\subsubsection*{Continuous predictor variables}
\label{subsubsec:chap05-continuous-covars}

\def\arcgis{\href{
http://resources.arcgis.com/en/help/main/10.1/index.html#/How_Topo_to_Raster_works/009z0000007m000000/}{ 
ArcGIS}}

The less detailed DEM (\demOld) is the hole-filled SRTM DEM version~\num{4} \cite{JarvisEtAl2008} 
(\autoref{fig:chap05-dem-old}). The spatial sampling support of the SRTM DEM is \SI{1}{\arcsecond} 
...
produced by interpolating contour lines with vertical spacing of \SI{10}{\m} along with data about the drainage 
network, lakes and peaks digitized from topographic maps with \scale{25000} (\autoref{fig:chap05-dem-new}). 
Interpolation to \SI{5}{\m} pixel size was performed using a hydrologically correct algorithm implemented in 
\arcgis{} software by ESRI \cite{Hutchinson1989}. Contour line artefacts were minimized using a seven by seven 
low-pass filter (\grass{r.neighbors}). The window size was chosen such that the smoothed DEM best matched the 
original contour map while also respecting the original drainage network pattern.

\input{chap/tab/chap01TAB1.tex}

\input{chap/tab/chap01TAB2.tex}

\input{chap/tab/chap01TAB3.tex}
...
$\texttt{NOR} = abs(\ang{180} - \texttt{ASP})$. \twi{} and \spi{} were calculated using \slp{} calculated with 
different window sizes, and \acc{} calculated using \grass{r.watershed}. \tpi{} was calculated using 
\saga{ta\_morphometry} with the same seven window sizes. The combination of DEM derivatives (\elev, \slp, \nor, 
\twi, \spi, and \tpi) and window sizes yielded $p = 36$ continuous predictor variables from each DEM.
...
(\grass{i.landsat.toar}). The more detailed satellite image comes from the RapidEye constellation (available at 
...
\begin{equation}\label{eq:chap05-lmm}
...
\autoref{sec:chap05-intro} that each covariate is available at two levels of spatial detail, hence $2^5$ 
...
\emph{model series plot} (\Rpackage{pedometrics}, \citeonline{Samuel-RosaEtAl2014c}). Pedological evaluation of 
predictor variables included in the models was omitted because this was beyond our objectives.

The multiple linear regression model calibrated using only the less detailed covariates, which we call the 
\emph{baseline} model, and the multiple linear regression model with the highest ${R}^{2}_{adj}$, which we call 
the \emph{best performing} model, were extended to linear mixed models of spatial variation 
(\autoref{eq:chap05-lmm}) for each soil property. Estimation of the parameters of the linear mixed models was 
performed using residual (restricted, marginal) maximum likelihood (REML) \cite{RibeiroEtAl2001, LarkEtAl2004}. 
The spatial correlation function adopted was the exponential function (this is equivalent to the Matérn 
correlation function with smoothness parameter $\nu = 0.5$ \cite{Stein1999}).

\subsubsection*{Model validation}
\label{subsec:chap05-validation}
...
calibrated using the two versions of the land use map (\texttt{land}). All green cells are on the right half of 
the plot between rankings \num{1} and \num{16} (see the x axis). The four lower rows show that the green cells 
of the other four covariates are distributed along the entire ranking range (from \num{1} to \num{32}). This 
means that the relative benefit of calibrating a SOC model with a more detailed land use map is larger compared 
to that of using a more detailed version of the other covariates.
...
fourth row) is not as beneficial as using \texttt{land}, \texttt{geo} or \texttt{dem}, but more beneficial than 
using \texttt{sat}. Because the covariates were ranked based on the mean rankings, the covariate displayed in 
the top row of each model series plot is the one which resulted in the largest improvement of the prediction 
accuracy when the more detailed version was used to calibrate the model -- for SOC this is the land use map.
...
similar for both \emph{baseline} and \empt{best performing} linear mixed models, which is also true for SOC 
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{90mm}
  \subcaption{}
  \includegraphics[width=90mm]{fig/chap01FIG6a} 
 \end{minipage}
 \begin{minipage}[b]{90mm}
  \subcaption{}
  \includegraphics[width=90mm]{fig/chap01FIG6b}
 \end{minipage}
 \begin{minipage}[b]{90mm}
  \subcaption{}
  \includegraphics[width=90mm]{fig/chap01FIG6c}
 \end{minipage}
 \caption{Experimental variogram (dots) and REML fit of the linear mixed models (line) for CLAY (a), SOC (b), 
 and ECEC (c). Left -- baseline model. Right -- best performing model.}
 \label{fig:chap05-lmm}
\end{figure}
...
SOC maps (\autoref{fig:chap05-soc-best-pred} and \autoref{fig:chap05-soc-best-var}) show peculiar features in 
...
Predicted ECEC (\autoref{fig:chap05-ecec-base-pred} and \autoref{fig:chap05-ecec-best-pred}) had a large 
...
eastern parts the influence of both covariates create an irregular pattern in the spatial distribution of ECEC. 
It is also in these parts that the largest prediction error standard deviations occur, following the spatial 
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-clay-base-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7a}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-clay-best-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7d}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-soc-base-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7b}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-soc-best-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7e}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-ecec-base-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7c}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-ecec-best-pred}
  \centering
  \includegraphics[width=63mm]{fig/chap01FIG7f}
 \end{minipage}
 \caption{Predicted values for CLAY (\si{\gram\per\kilo\gram}) (a, b), SOC (\si{\gram\per\kilo\gram}) (c, d), 
 and ECEC (\si{\milli\mole\per\kilo\gram}) (e, f) using the \emph{baseline} (left) and \emph{best performing}
 (right) linear mixed models.}
 \label{fig:chap05-kriging}
\end{figure}
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-clay-base-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8a}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-clay-best-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8d}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-soc-base-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8b}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-soc-best-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8e}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-ecec-base-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8c}
 \end{minipage}
 \begin{minipage}[b]{63mm}
  \subcaption{}
  \label{fig:chap05-ecec-best-var}
  \centering
  \includegraphics[width=60mm]{fig/chap01FIG8f}
 \end{minipage}
 \caption{Prediction error standard deviations for CLAY (\si{\gram\per\kilo\gram}) (a, b), SOC 
 (\si{\gram\per\kilo\gram}) (c, d), and ECEC (\si{\milli\mole\per\kilo\gram}) (e, f) using the \emph{baseline}
 (left) and \emph{best performing} (right) linear mixed models.}
 \label{fig:chap05-kriging-variance}
\end{figure}
...
the size of the minimum legible delineation (see \citeonline{HenglEtAl2006a} for a discussion on effective 
...
findings by \citeonline{ThompsonEtAl2001} and \citeonline{KimEtAl2014}.
...
correlation that is already high in the \emph{baseline} model. \citeonline{CavazziEtAl2013} suggest that the 
