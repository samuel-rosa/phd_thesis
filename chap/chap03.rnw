...
\begin{figure}[!ht]
\centering
<<location, fig = TRUE, echo = FALSE, include = FALSE>>=
# Start GRASS
...
# Load and process polygon with the boundary of Brazilian territory
brazil <- raster::shapefile("~/projects/dnos-sm-rs/dnos-sm-rs-data/boundaries/brasil.shp")
bb <- sp::bbox(brazil)
bb[1, 2] <- -34.7
slot(brazil, "bbox") <- bb

# Load and process polygon with the boundary of the Paraná Sedimentary Basin
parana <- raster::shapefile("~/projects/dnos-sm-rs/dnos-sm-rs-data/boundaries/parana-basin.shp")
parana <- sp::spTransform(parana, sp::proj4string(brazil))
parana <- list("sp.polygons", parana, col = "gray", fill = "gray", alpha = 0.6, first = FALSE)

# Prepare points for reference cities
...
col <- c("blue", rep("black", nrow(pts) - 1))
colnames(pts) <- c("long", "lat")
sp::coordinates(pts) <- ~ long + lat
sp::proj4string(pts) <- sp::proj4string(brazil)
pts <- list("sp.points", pts, pch = c(8, rep(20, length(pts) - 1)), cex = 0.5, col = col)

# set map colors
brazil$UF_05 <- as.factor(brazil$UF_05)
rs <- rep("lemonchiffon", length(brazil$UF_05))
rs[which(brazil$UF_05 == "RS")] = "lightgreen"

# prepare spplot of brazil
brazil <- sp::spplot(
  brazil, zcol = "UF_05", aspect = "iso", col = "gray", 
  scales = list(draw = TRUE, x = list(at = seq(-70, -35, 5)), y = list(at = seq(-30, 5, 5))),
  col.regions = colorRampPalette(rs)(27), colorkey = FALSE, cex = 0.3, sp.layout = list(parana, pts),
  par.settings = list(
    fontsize = list(text = 12, points = 8), layout.widths = list(left.padding = 0, right.padding = 0),
    layout.heights = list(top.padding = 0, bottom.padding = 0)),
  panel = function(x, y, ...) {
    sp::panel.polygonsplot(x, y, ...)
    lattice::panel.abline(h = seq(-30, 0, 5), v = seq(-70, -40, 5), col = "gray", lty = "dashed", lwd = 0.5)
    lattice::panel.text(x = -53.790215, y = -29.657668, "Santa Maria", pos = 2, col = "blue")
    lattice::panel.text(x = -47.887768, y = -15.788838, "Brasília", pos = 4)
    lattice::panel.text(x = -46.665337, y = -23.536445, "São Paulo", pos = 4)
    lattice::panel.text(x = -51.211134, y = -30.032484, "Porto Alegre", pos = 4)
    lattice::panel.text(x = -59.992370, y = -3.080757, "Manaus", pos = 4)
  }
)
...
\begin{figure}[!ht]
\centering
<<perspective, echo = FALSE, fig = TRUE, include = FALSE>>=
# Start GRASS
invisible(capture.output(
  spgrass6::initGRASS(
    gisBase = "/usr/lib/grass64/", gisDbase = path.expand("~/dbGRASS"),
...
# Prepare figure with study area in perspective
system("g.remove MASK", ignore.stdout = TRUE, ignore.stderr = TRUE)
system("g.region rast=ELEV_10", ignore.stdout = TRUE, ignore.stderr = TRUE)
output <- path.expand("~/projects/phd_thesis/chap/fig/chap03-perspective")
cmd <- paste(
  "m.nviz.image elevation_map=ELEV_10 color_map=rapideye_composite arrow_position=600,40 ",
  "vline=manual_limit_carta,manual_sublimit_carta vline_color=blue,red vline_width=2,2 ",
  "vline_mode=surface,surface vline_height=0,0 vline_position=0,0,0,0,0,0 perspective=13 ",
  "light_position=0.85,0.25,0.8 position=0.75,1 height=4500 zexag=3.0 format=tif output=", 
  output, sep = "")
system(cmd, ignore.stdout = TRUE, ignore.stderr = TRUE)

# Convert tif file to png
cmd <- paste("convert ~/projects/phd_thesis/chap/fig/chap03-perspective.tif ",
             "convert ~/projects/phd_thesis/chap/fig/chap03-perspective.png", sep = "")
system(cmd, ignore.stdout = TRUE, ignore.stderr = TRUE)
system("rm ~/projects/phd_thesis/chap/fig/chap03-perspective.tif")
@
...
A bacia do DNOS já foi intensamente ocupada em tempos pretéritos para a produção agrossilvopastoril. Isso é 
evidenciado pela grande quantidade, extensão e boa distribuição da rede viária em toda a sua área. Além disso, 
a área é cortada por uma estrada férrea e avizinha uma rodovia federal, ambas muito movimentadas. Entretanto, 
nas últimas décadas, muitos caminhos internos das propriedades rurais foram desativados, fruto do abandono de 
áreas de produção, a maioria delas localizada nos topos dos morros, patamares do Rebordo ou no fundo de vales, 
onde a manutenção dos caminhos é muito onerosa, especialmente quando distantes da sede das propriedades 
(\autoref{fig:chap05-land-use} e \autoref{fig:chap05-sat-image}). No passado, essas estradas internas 
possibilitavam o trânsito de pessoas e o transporte da produção agrossilvopastoril, a qual era comercializada 
...
localizada no interior de áreas de floresta natural regenerada, o que dificulta a sua identificação com 
imagens aéreas ou orbitais. Em alguns casos, esses caminhos são utilizados em atividades de turismo, 
especialmente para caminhadas a pé, ou atividades esportivas com motocicletas (trilhas).
...
região do Rebordo do Planalto da Bacia do Paraná, o uso da terra para fins agrossilvopastoris foi 
...
inúmeras décadas \cite{SamuelRosaEtAl2011a}.
...
superficiais são destruídas \cite{ScheneiderEtAl1978, HackEtAl2005}. O resultado desse processo é a perda da 
qualidade do solo e, consequentemente, da produtividade da floresta \cite{KonigEtAl2002}.
...
mais profundo e menos pedregoso do que em áreas de floresta natural e vegetação secundária. Além disso, são 
encontrados fragmentos de carvão e formações de microrrelevo devido ao uso de implementos agrícolas para o 
revolvimento do solo na maior parte desses locais, evidenciando seu uso agrícola num passado recente. Essas 
áreas podem ser divididas entre aquelas de campo sujo (pastagens naturais mal manejadas, com predomínio de 
vegetação de porte herbáceo-arbustivo) e campo limpo (pastagens naturais e perenes bem manejadas) 
...
nas áreas de floresta e sob vegetação secundária, a dinâmica de uso dessas áreas ao longo do tempo é 
bastante complexa, dificultando o estabelecimento de relações diretas com a maior parte das características do 
solo. Entretanto, sob o ponto de vista da reserva de nutrientes e matéria orgânica, o solo pode ser 
considerado pobre, uma vez que a exploração pecuária é totalmente extrativista e extensiva.

As atividades agrossilvopastoris que ocupam menor extensão territorial são a agricultura e a silvicultura. As 
áreas de lavoura estão dispersas em toda a área, geralmente localizadas em terrenos de menor declividade e 
solo medianamente profundo. Entretanto, algumas áreas de produção agrícola possuem declives superiores a 
\SI{50}{\percent} e solo raso e pedregoso \cite{SamuelRosaEtAl2011a}. Em qualquer das situações, as condições 
de degradação do solo costumam ser bastante avançadas devido à ausência de práticas conservacionistas, exceto 
por algumas áreas de produção olerícola. Assim como as áreas de agricultura, as florestas plantadas 
...
MiguelEtAl2014}. Soma-se a isso os resíduos domésticos e cloacais historicamente despejados nos cursos de água 
devido à falta de coleta e tratamento \cite{Goldani2006}. Quanto às demais obras de engenharia, destacam-se os 
reservatórios de água, a maioria deles de pequena extensão, utilizados para a dessedentação animal. Como os 
cursos de água de maior volume estão localizados na porção sul da área, a maior parte dos reservatórios de 
água está na porção norte \cite{SamuelRosaEtAl2011a}. Além disso, a menor permeabilidade do solo e do substrato 
rochoso, bem como da condição topográfica, favorecem essa característica.

\section{PEDOLOGIA}

O solo da área de estudo possui características com forte dependência do material de origem 
\cite{NascimentoEtAl2010} que, conforme descrito anteriormente, é um importante condicionante da geomorfologia 
e da 
