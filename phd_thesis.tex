...
\def\gates{%
\href{http://www.gatesfoundation.org/what-we-do/global-development/agricultural-development%
}{Bill \& Melinda Gates}%
}%
...
(\texttt{pedometrics}) containing miscellaneous functions, the second (\texttt{spsann}) designed for the 
optimization of sample configurations using spatial simulated annealing. The case studies demonstrate that 
existing covariates are suitable for calibrating soil spatial models, and using more detailed covariates 
results in only a modest increase in 
the prediction accuracy that may not outweigh the extra costs. More efficient means of increasing prediction 
